...
* 로비 또는 미션 전환 후에도 유지할 필요가 있는 장비 기반 Ability
...
* Client OnRep\_PlayerState 시 InitAbilityActorInfo 호출 여부
...
* 장비 변경으로 부여된 Ability가 서버와 클라이언트에서 일관되게 보이는가?
* Late Join 시 장비 상태와 GAS 상태가 올바르게 복원되는가?

---

## Attribute

HP, Shield, Energy, MoveSpeed 등 게임플레이 수치는 AttributeSet으로 관리한다.
...
* 장비 파츠로 인한 지속 스탯 변화는 Infinite GE를 우선 검토한다.
...
* 장비 장착 중 유지되는 MaxHealth 증가
* 장비 장착 중 유지되는 MoveSpeed 증가
* 패시브 효과
...
GameplayTag는 상태, 조건, 차단 조건, 입력, 장비 상태를 표현한다.
...
Equipment.Slot.Legs
Equipment.Slot.Arms
Equipment.Slot.WeaponRight
Equipment.Slot.WeaponLeft
Equipment.Slot.Accessories
Equipment.Status.Equipped

Cooldown.Weapon.Primary
Cooldown.Weapon.Secondary
Cooldown.Movement.Dash

Input.Weapon.Primary
Input.Weapon.Secondary
Input.Movement.Dash
```
...
이 프로젝트에서 메카 파츠 장착은 GAS와 직접 연결된다.
...
* Late Join 시 장비 상태와 부여된 Ability/Effect 상태가 올바르게 복원되어야 한다.
